...
const PokemonCard = (pokemon) => {
    const { data, isLoading, error } = useGetPokemonDetailsQuery(pokemon.pokemon.url);
    const [ Color, setColor] = useState("");

    if (!isLoading) {
        return <div className={`${styles.Card} ${styles[`--${data.types[0].type.name}`]}`}>
            <h1 className={styles.Card__Name}>{data.name}</h1>
            <p className={styles.Card__Type1}>{data.types[0].type.name}</p>
            {data.types[1] != null && <p className={styles.Card__Type2}>{data.types[1].type.name}</p>}
            <h2 className={styles.Card__Id}>#{+data.id<10 ? "00" : +data.id < 100 ? "0" : ""}{data.id}</h2>
            ...
            <img className={styles.Card__Image} src={data.sprites.front_default} alt={data.name} />
        </div>
    } else {
